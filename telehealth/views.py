...
from .serializers import (
    ...
    TelehealthSessionCreateSerializer,
    TelehealthTranscriptSerializer,
)
import logging
import uuid
...
class TelehealthSessionViewSet(viewsets.ModelViewSet):
    ...
    def emergency_sessions(self, request):
        ...
        sessions = TelehealthSession.objects.filter(
            Q(patient=user) | Q(therapist=user), is_emergency=True
        ).order_by("-scheduled_at")
    ...
    def transcripts(self, request):
        ...
        transcripts = (
            TelehealthTranscript.objects.select_related(
                "session", "patient", "therapist", "created_by"
            )
            .filter(
                Q(session__therapist=request.user)
    ...
    def create_emergency(self, request):
        ...
        if request.user.role not in ["admin", "therapist", "staff"]:
            return Response(
                {
                    "error": (
                        "Only therapists, staff, and admins can create"
                        " emergency sessions."
                    )
                },
                status=status.HTTP_403_FORBIDDEN,
