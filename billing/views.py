...
class BillViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if user.role in ['admin', 'staff']:
            queryset = Bill.objects.select_related('patient', 'created_by').prefetch_related('payments')
    ...
    def summary(self, request):
        ...
        total_overdue = bills.filter(status='overdue').aggregate(Sum('amount'))['amount__sum'] or 0
...
class PaymentViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if user.role in ['admin', 'staff']:
            ...
            return Payment.objects.filter(bill__patient__user=user).select_related('bill', 'recorded_by')
