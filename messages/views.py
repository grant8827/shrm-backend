    ...
    def create(self, request):
        try:
            ...
            if not recipients.exists():
                ...
                return Response({'error': 'Invalid recipient IDs'}, status=status.HTTP_400_BAD_REQUEST)
            
            all_participant_ids = set([str(request.user.id)] + [str(r.id) for r in recipients])
            logger.info(f"Participant IDs: {all_participant_ids}")
            
            # Find existing thread with same participants
            thread = None
            for existing_thread in MessageThread.objects.filter(participants=request.user):
                thread_participant_ids = set([str(p.id) for p in existing_thread.participants.all()])
