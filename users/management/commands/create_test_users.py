        ...
        try:
            with transaction.atomic():
                ...
                if created:
                    ...
                    self.stdout.write(
                        self.style.SUCCESS(
                            f"✓ Created therapist user: {therapist_user.email}"
                        )
                    )
                ...
                if created:
                    ...
                    self.stdout.write(
                        self.style.SUCCESS(
                            f"✓ Created client user: {client_user.email}"
                        )
                    )
                ...
                if created:  # Only create patient if client was just created
                    ...
                    if patient_created:
                        self.stdout.write(
                            self.style.SUCCESS(
                                f"✓ Created patient record for: {client_user.email}"
                            )
                        )
                ...
                users_info = [
                    (
                        "Admin",
                        "admin@theracare.local",
                        "AdminPass123!",
                        "Full system access",
                    ),
                    (
                        "Therapist",
                        "dr.smith@theracare.local",
                        "TherapistPass123!",
                        "Clinical access",
                    ),
                    (
                        "Staff",
                        "staff@theracare.local",
                        "StaffPass123!",
