...
class IsTherapistStaffOrAdmin(permissions.BasePermission):
    ...
    def has_permission(self, request, view):
        ...
        return (
            request.user and 
            request.user.is_authenticated and 
            request.user.role in ['therapist', 'staff', 'admin']
        )
...
class IsAdminOrSelf(permissions.BasePermission):
    ...
    message = "You can only access your own information unless you are an administrator."
...
class IsPatientOwnerOrTherapist(permissions.BasePermission):
    ...
    def has_permission(self, request, view):
        ...
        return (
            request.user and 
            request.user.is_authenticated and 
            request.user.role in ['admin', 'therapist', 'staff', 'client']
        )
...
class IsSOAPNoteAuthorOrTherapist(permissions.BasePermission):
    ...
    def has_object_permission(self, request, view, obj):
        ...
        if request.user.role == 'client':
            ...
            return (
                hasattr(soap_note.patient, 'user') and 
                soap_note.patient.user == request.user
            )
...
class IsBillingAuthorized(permissions.BasePermission):
    ...
    def has_permission(self, request, view):
        ...
        return (
            request.user and 
            request.user.is_authenticated and 
            request.user.role in ['admin', 'staff', 'client']
        )
    ...
    def has_object_permission(self, request, view, obj):
        ...
        if request.user.role == 'client':
            if hasattr(billing, 'patient'):
                return (
                    hasattr(billing.patient, 'user') and 
                    billing.patient.user == request.user
                )
