    ...
    def validate(self, attrs):
        ...
        if not username or not password:
            raise serializers.ValidationError(
                "Both username and password are required.", code="missing_credentials"
            )
        ...
        try:
            ...
            logger.warning(
                ...
                extra={
                    ...
                    "user_agent": self.context.get("request").META.get(
                        "HTTP_USER_AGENT"
                    ),
        ...
        if not user.is_active:
            ...
            raise serializers.ValidationError(
                "Account is inactive. Please contact support.", code="account_inactive"
            )
            ...
            try:
                ...
                logger.warning(
                    ...
                    extra={
                        ...
                        "ip_address": self.context.get("request").META.get(
                            "REMOTE_ADDR"
                        ),
                        "user_agent": self.context.get("request").META.get(
                            "HTTP_USER_AGENT"
        ...
        try:
            ...
            logger.error(
                f"Error decrypting first_name during login for user {user.id}: {e}"
            )
        ...
        try:
            ...
            logger.error(
                f"Error decrypting last_name during login for user {user.id}: {e}"
            )
    ...
    def create(self, validated_data):
        ...
        logger.info(
            ...
            extra={
                ...
                "role": user.role,
                "created_by": (
                    str(self.context.get("request").user.id)
...
class CompleteRegistrationSerializer(serializers.Serializer):
    ...
    def validate_token(self, value):
        ...
        try:
            ...
            if not token.is_valid():
                raise serializers.ValidationError(
                    "This registration link has expired or has already been used."
                )
            return value
    ...
    def create(self, validated_data):
        ...
        try:
            ...
            logger.error(
                f"Failed to link patient record for registration token {token.id}: {e}"
            )
