...
admin_user = User.objects.filter(role='admin').first() or User.objects.filter(role='therapist').first()
...
for patient_data in test_patients:
    ...
    print(f"✓ Created patient: {patient_data['first_name']} {patient_data['last_name']} (#{patient.patient_number})")
    created += 1
