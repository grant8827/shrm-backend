...
class SOAPNoteViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        queryset = SOAPNote.objects.select_related(
    ...
    def stats(self, request):
        ...
        stats = {
            ...
            "completed_this_week": queryset.filter(
                status="finalized", session_date__gte=start_of_week
            ).count(),
            "draft": queryset.filter(status="draft").count(),
            "overdue": queryset.filter(
                status="draft", created_at__lt=cutoff_48h
            ).count(),
        }
