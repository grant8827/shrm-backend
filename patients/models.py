...
class Patient(models.Model):
    ...
    assigned_therapists = models.ManyToManyField(
        User, through="PatientTherapistAssignment", related_name="assigned_patients"
    )
    ...
    def generate_patient_number(self):
        ...
        last_patient = (
            Patient.objects.filter(patient_number__startswith=f"P{year_suffix}")
...
class InsuranceInformation(models.Model):
    ...
    insurance_type = models.CharField(
        max_length=20, choices=InsuranceType.choices, default=InsuranceType.PRIMARY
    )
