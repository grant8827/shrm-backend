...
SECRET_KEY = config(
    "SECRET_KEY", default="django-insecure-theracare-dev-key-change-in-production"
)
...
if DATABASE_URL:
    ...
    db_port = config(
        "DB_PORT",
        default=config(
            "PGPORT", default="3306" if DB_CONNECTION == "mysql" else "5432"
        ),
    )
...
HIPAA_SETTINGS = {
    "ENCRYPTION_KEY": config(
        "ENCRYPTION_KEY", default="hipaa-encryption-key-change-in-production"
    ),
...
if DEBUG:
    ...
    CSP_MEDIA_SRC = (
        ...
        "blob:",
        "mediastream:",
    )  # Allow camera/microphone streams
else:
    ...
    CSP_CONNECT_SRC = (
        ...
        "wss://shrm-backend-production.up.railway.app",
        "ws:",
        "wss:",
    )
...
ALLOW_INMEMORY_CHANNEL_LAYER = config(
    "ALLOW_INMEMORY_CHANNEL_LAYER", default=False, cast=bool
)
MANAGEMENT_COMMANDS_ALLOWLIST = {
    "check",
    "makemigrations",
    "migrate",
    "collectstatic",
...
IS_MANAGEMENT_COMMAND = (
    len(sys.argv) > 1 and sys.argv[1] in MANAGEMENT_COMMANDS_ALLOWLIST
)
...
if (
    ...
    and not REDIS_URL
    and not ALLOW_INMEMORY_CHANNEL_LAYER
):
